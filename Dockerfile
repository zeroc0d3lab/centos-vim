...
# -) vundle + themes
#-----------------------------------------------------------------------------
RUN curl -sSL https://github.com/vim/vim/archive/v${VIM_VERSION}.zip -o /opt/vim.zip \
    && cd /opt \
    && unzip /opt/vim.zip \
    && mv /opt/vim-${VIM_VERSION} /opt/vim
...
RUN git clone https://github.com/zeroc0d3/vim-ide.git /opt/vim-ide \
